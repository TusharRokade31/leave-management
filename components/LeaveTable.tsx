      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden border border-gray-100 dark:border-slate-800/60">
          ...
          <table className="w-full border-separate border-spacing-0">
            <thead>
              <tr className="bg-gray-50 dark:bg-slate-800/40">
                <th className="sticky left-0 top-0 bg-gray-50 dark:bg-slate-800 border-b border-r border-gray-100 dark:border-slate-800 p-4 min-w-[80px] z-30 text-[10px] font-black uppercase text-slate-400 tracking-widest">
                  ID
                </th>
                <th className="sticky left-[80px] top-0 bg-gray-50 dark:bg-slate-800 border-b border-r border-gray-100 dark:border-slate-800 p-4 min-w-[180px] z-30 text-[10px] font-black uppercase text-slate-400 tracking-widest text-left">
                  Employee
                </th>
                {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => (
                  <th
                    key={day}
                    className={`border-b border-r border-gray-100 dark:border-slate-800 p-2 min-w-[50px] text-center transition-colors ${
                      isWeekendDay(day) 
                  ...
                  >
                    ...
                    <div className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">
                      {getDayOfWeek(day)}
                    </div>
                  </th>
                ))}
                  ...
                  {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
                    ...
                    return (
                      <td
                        key={day}
                        className={`
                          border-b border-r border-gray-50 dark:border-slate-800/50 p-1 text-center text-[10px] font-black transition-all
                          ${isWeekend ? "bg-red-50/20 dark:bg-red-950/10" : ""}
                          ${isLeaveDay && leaveForDay?.type === 'WORK_FROM_HOME' ? "bg-blue-100/50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300" : ""} 
                          ${isLeaveDay && leaveForDay?.type !== 'WORK_FROM_HOME' ? "bg-emerald-100/50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-300" : ""}
                        `}
                      >
                        <span className={isLeaveDay ? "scale-110 inline-block drop-shadow-sm" : "opacity-0"}>
                          {symbol}
                        </span>
                      </td>
                    );
        ...
        <div className="p-8 bg-gray-50/50 dark:bg-slate-900/80 border-t border-gray-100 dark:border-slate-800 backdrop-blur-md">
          ...
          <div className="flex flex-wrap gap-x-10 gap-y-4">
            <LegendItem color="bg-emerald-100/60 dark:bg-emerald-900/40 border-emerald-200/50 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400" label="Physical Leave" symbol="F/H/E/L" />
            <LegendItem color="bg-blue-100/60 dark:bg-blue-900/40 border-blue-200/50 dark:border-blue-800 text-blue-600 dark:text-blue-400" label="Remote Work" symbol="WFH" />
            <LegendItem color="bg-red-50/50 dark:bg-red-950/30 border-red-100 dark:border-red-900" label="Weekend" symbol="" />
          </div>
        </div>
      </div>
    </div>
...
const LegendItem = ({ color, label, symbol }: { color: string; label: string; symbol: string }) => (
  <div className="flex items-center gap-3 group cursor-default">
    ...
    <span className="text-[10px] font-black uppercase text-slate-600 dark:text-slate-400 tracking-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
      {label}
    </span>
  </div>
);
