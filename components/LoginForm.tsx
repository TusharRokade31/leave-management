  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-4 transition-colors">
      <div className="absolute top-6 right-6">
        <button type="button" onClick={toggleTheme} className="p-3 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-xl shadow-sm text-gray-700 dark:text-slate-300">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-8 w-full max-w-md border dark:border-slate-800">
        ...
        {loginMode !== 'forgot' ? (
          <div className="flex gap-2 mb-6 bg-gray-100 dark:bg-slate-800 p-1 rounded-lg">
            <button type="button" onClick={() => switchMode('password')} className={`flex-1 py-2 px-4 rounded-md font-medium transition-all ${loginMode === 'password' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-gray-500'}`}>
              <Lock className="w-4 h-4 inline mr-2" /> Password
            </button>
            <button type="button" onClick={() => switchMode('otp')} className={`flex-1 py-2 px-4 rounded-md font-medium transition-all ${loginMode === 'otp' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-gray-500'}`}>
              <KeyRound className="w-4 h-4 inline mr-2" /> OTP
            </button>
          </div>
        ) : (
          <button type="button" onClick={() => switchMode('password')} className="flex items-center text-sm text-indigo-600 dark:text-indigo-400 mb-6 hover:opacity-80">
            <ArrowLeft size={16} className="mr-1" /> Back to Login
          </button>
        )}

        {/* Unified Form for Keyboard Navigation */}
            ...
            <>
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-2 border dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" 
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <button type="button" onClick={() => switchMode('forgot')} className="text-xs text-indigo-600 hover:underline">Forgot Password?</button>
                </div>
              ...
              <button type="submit" disabled={isLoading} className="w-full bg-indigo-600 text-white py-3 rounded-lg hover:bg-indigo-700 font-medium disabled:opacity-50">
            ...
            <>
              <div>
                ...
                <input 
                  ...
                  disabled={otpSent} 
                  className="w-full px-4 py-2 border dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg outline-none disabled:bg-slate-50 dark:disabled:bg-slate-900" 
                  placeholder="user@company.com" 
                />
              </div>

              {!otpSent ? (
                <button type="submit" disabled={isLoading || !otpForm.email} className="w-full bg-indigo-600 text-white py-3 rounded-lg font-medium">
                ...
                <>
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-2 border dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg text-center text-xl tracking-widest outline-none" 
                      placeholder="000000" 
                    />
                  </div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <input 
                            ...
                            className="w-full px-4 py-2 border dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-lg outline-none" 
                  ...
                  <button type="submit" disabled={isLoading || otpForm.otp.length !== 6} className="w-full bg-indigo-600 text-white py-3 rounded-lg font-medium">
                    {isLoading ? 'Verifying...' : loginMode === 'forgot' ? 'Update Password' : 'Login with OTP'}
                  </button>
                  <button type="button" onClick={() => setOtpSent(false)} className="w-full text-sm text-gray-500 hover:text-indigo-600">Resend OTP</button>
                </>
              )}
            </>
          )}
